...
func create_weapon():
	...
	var armor_multiplier = PlayerData.armor_multiplier
	var level_multiplier = 1 + ((level - 1) * 0.42) ** 6
	var qualities = weapon_qualities.keys()
	var random_index = randi() % qualities.size()
	var weapon_quality = qualities[random_index]
	var weapon_damage = weapon_qualities[weapon_quality] * base_damage * level_multiplier * armor_multiplier
	var weapon = {
